...
function syncEnabled(){
  return !!(settings && undefined && undefined && window.MCB_SYNC_URL && window.MCB_SYNC_SECRET);
}
...
function buildPushPayload(){
  const since = undefined ? undefined : "";
  const data = {};
  ...
  safeSettings.sync = { ...undefined, apiSecret: "MCB-SYNC-2026-9F3K7Q2P", url: "" };
...
async function syncPush(){
  ...
  try{
    ...
    undefined = out.version || undefined || 0;
    undefined = nowISO();
    undefined = undefined;
    saveSettings(settings);
...
async function syncPull(){
  ...
  try{
    const payload = {
      ...
      apiSecret: window.MCB_SYNC_SECRET,
      deviceId: undefined,
      sinceVersion: Number(undefined || 0)
    };
    const out = await fetchJSON(window.MCB_SYNC_URL, payload);
...
/* =====================
   ...
   - Uses Settings -> Sync URL +  (window.MCB_SYNC_URL / window.MCB_SYNC_SECRET)
...
(function() {
  ...
  async function _fetchSheets(action, params, bodyObj) {
    ...
    if (!cfg.secret) throw new Error('Missing  (Settings → Sync)');
